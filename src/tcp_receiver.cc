...
TCPReceiverMessage TCPReceiver::send( const Writer& inbound_stream ) const
{
  ...
  receivermessage.window_size = inbound_stream.available_capacity() > UINT16_MAX ? UINT16_MAX : inbound_stream.available_capacity();

  uint64_t num_bytes = inbound_stream.bytes_pushed();
  if(is_syn) {
    if(is_fin && inbound_stream.is_closed()) {  num_bytes++;  }
